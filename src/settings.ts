...
Behalte Bildverweise im Obsidian-Format (\`![[Dateiname.ext]]\`) exakt so bei.
...
export class AnkiGeneratorSettingTab extends PluginSettingTab {
	...
	display(): void {
		...
		containerEl.createEl('h3', { text: 'Google Gemini (Primär)' }); new Setting(containerEl).setName('Gemini API Key').addText(text => text.setPlaceholder('Gib deinen Schlüssel ein...').setValue(this.plugin.settings.geminiApiKey).onChange(async (value) => { this.plugin.settings.geminiApiKey = value; await this.plugin.saveSettings(); })); containerEl.createEl('h3', { text: 'Ollama (Fallback)' }); new Setting(containerEl).setName('Ollama als Fallback aktivieren').setDesc('Wenn Gemini nicht erreichbar ist, wird versucht, Ollama lokal zu verwenden.').addToggle((toggle: ToggleComponent) => { toggle.setValue(this.plugin.settings.ollamaEnabled).onChange(async (value) => { this.plugin.settings.ollamaEnabled = value; await this.plugin.saveSettings(); this.display(); }); }); if (this.plugin.settings.ollamaEnabled) { new Setting(containerEl).setName('Ollama API Endpunkt').setDesc('Die URL deiner lokalen Ollama-Instanz.').addText(text => text.setPlaceholder('z.B. http://localhost:11434/api/generate').setValue(this.plugin.settings.ollamaEndpoint).onChange(async (value) => { this.plugin.settings.ollamaEndpoint = value; await this.plugin.saveSettings(); })); new Setting(containerEl).setName('Ollama Modellname').setDesc('Der Name des Modells (z.B. llama3, mistral).').addText(text => text.setPlaceholder('z.B. llama3').setValue(this.plugin.settings.ollamaModel).onChange(async (value) => { this.plugin.settings.ollamaModel = value; await this.plugin.saveSettings(); })); } containerEl.createEl('h3', { text: 'Anki' }); new Setting(containerEl).setName('Anki Hauptdeck').addText(text => text.setPlaceholder('z.B. Medizin').setValue(this.plugin.settings.mainDeck).onChange(async (value) => { this.plugin.settings.mainDeck = value; await this.plugin.saveSettings(); })); new Setting(containerEl).setName('Name für Basic-Kartentyp').setDesc('Der exakte Name des "Basic" Notiztyps in Anki.').addText(text => text.setValue(this.plugin.settings.basicModelName).onChange(async (value) => { this.plugin.settings.basicModelName = value; await this.plugin.saveSettings(); })); new Setting(containerEl).setName('Name für Lückentext-Kartentyp').setDesc('Der exakte Name des "Cloze" / "Lückentext" Notiztyps in Anki.').addText(text => text.setValue(this.plugin.settings.clozeModelName).onChange(async (value) => { this.plugin.settings.clozeModelName = value; await this.plugin.saveSettings(); })); containerEl.createEl('h3', { text: 'Prompt' }); new Setting(containerEl).setName('System Prompt').setDesc('Die Anweisung für die KI. {{noteContent}} und {{existingCards}} werden automatisch ersetzt.').addTextArea(text => text.setValue(this.plugin.settings.prompt).onChange(async (value) => { this.plugin.settings.prompt = value; await this.plugin.saveSettings(); }).inputEl.rows = 15);
